...
int main(int argc, char* argv[]) {
    ...
    for (int t = 1; t <= 5; ++t) {
        ...
        if (t == 1) { 
            ...
            // Worst-case của các thuật toán so sánh: Mảng nghịch thế (giảm dần).
    ...
    for (int t = 1; t <= 5; ++t) {
        ...
        if (t == 1 || t == 2) {
            ...
            // Các chuỗi cùng độ dài 100 này sau khi được đối thủ xếp từ điển, sẽ bị hàm countingSort dùng hàm swap() làm xáo trộn lại toàn bộ vị trí -> Sai kết quả (Wrong Answer).            for (int i = 0; i < N; i += 24) {
