...
3.  **Tối ưu hóa so sánh bằng mã hóa chuỗi (String Hashing/Word-packing):** Ép 8 ký tự `char` đầu tiên của chuỗi vào một biến kiểu số nguyên 64-bit (`uint64_t`). Khi so sánh tiền tố chuỗi ở những bước đầu tiên, thuật toán chỉ cần so sánh các số nguyên 64-bit bằng một lệnh CPU duy nhất thay vì duyệt qua từng ký tự một, giúp tăng tốc độ xử lý các chuỗi có tiền tố lặp lại lên gấp nhiều lần.
